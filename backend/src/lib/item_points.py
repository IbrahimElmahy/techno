...
_FAMILY_BY_CATEGORY = {
    ...
    "@@فئه عامه": NONE,
    "فئه عامه": NONE,
    # ── مالهاش نقط: قرار العميل ١٨ سبتمبر ٢٠٢٦ ────────────────────────────────
    # ملف نقاط البيع اللي كاتبه بإيده (`points.tsv`) فيه خمس عيال بس، كلها تكنو:
    # تكنو ثيرم · تكنو ثيرم معزول · جوان · ابيض ١١٤ · ابيض ١١٠. الباقي بضاعة
    # بيتاجر فيها مش بيصنّعها، والولاء على منتجه هو.
    #
    # **مكتوبين بالاسم واحد واحد، مش «أي تصنيف مش معروف = صفر».** التصنيف الجديد
    # اللي يتضاف بكرة لازم يطلع في «محتاج قرار» عشان حد يبصّ له — لو خلّينا
    # المجهول صفر، أول خط منتجات جديد هيدخل النظام وهو مش بيكسب ومحدش هيلاحظ.
    ...
    # الاستثمارى والرمادى — خطوط اقتصادية مش في جدول العميل.
    "الاستثماري الابيض": NONE, "استثماري بولي": NONE, "مواسيراستثماري": NONE,
    "الرمادي": NONE, "مواسيررمادي": NONE,
    # البولى ايثيلين: جدول العميل مافيهوش ولا سطر عنه — لا ماسورة ولا وصلة.
    ...
    # أدوات صحية (حوض · خلاط · شطافة · طقم دش) — القفيز جواها بيتمسك بالاسم فوق.
    "النواكل": NONE,
}
