...
def run(*, execute: bool) -> None:
    ...
    try:
        ...
        customer_group = in_group.get(CUSTOMER_GROUP, set())
        ...
        for code, expect_name, want in wanted:
            ...
            if _norm(c.name) != _norm(expect_name):
                ...
                continue
            # حارسين، كل واحد بيفحص الشرط اللي القرار اتبنى عليه:
            #
            # * «موظف» لازم يفضل ليه حساب مرتب **ومالوش** حساب عملاء — لو ظهر له
            #   حساب عملاء يبقى بقى عميل كمان، والكارت مش بتاعنا نحكم عليه.
            # * «تاجر» (الرجوع) لازم يفضل ليه حساب عملاء — ده كل سبب الرجوع.
            #
            # القسم الداخلي معفي من الاتنين — هو مش شخص أصلاً.
            if want == "employee":
                if _norm(c.name) not in payroll:
                    problems.append(
                        f"{code} «{c.name}»: مالوش حساب مرتب في a5 دلوقتي — اتخطّى")
                    continue
                if _norm(c.name) in customer_group:
                    problems.append(
                        f"{code} «{c.name}»: بقى ليه حساب تحت «العملاء» — اتخطّى")
                    continue
            if want == "trader" and _norm(c.name) not in customer_group:
                problems.append(f"{code} «{c.name}»: مالوش حساب تحت «العملاء» — اتخطّى")
                continue
        ...
        missed = [c for c in db.scalars(select(Customer).where(Customer.active.is_(True)))
                  if _norm(c.name) in payroll
                  and _norm(c.name) not in customer_group
