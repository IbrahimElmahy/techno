...
class ProductionState(str, enum.Enum):
    ...
    (`confirmed`)، وبعد ما الشغل يخلص بيتنفّذ (`done`) — وساعتها بس الخامات بتتصرف
    والتام بيدخل. من غير الفصل ده، أي ورقة بتتكتب بالنهار بتخصم مخزون مااتصرفش، واللي
    بيعدّل سطر بعد نص ساعة بيبقى بيعدّل حركة اتكتبت خلاص.
    """
