...
export function useTreasuryGate(enabled = true) {
  ...
  const ask = useCallback((o: TreasuryAsk, run: (id: number | null) => void) => {
    ...
    // **مافيش فلوس ⇒ مافيش سؤال عن خزنة.**
    //
    // المستند الآجل بالكامل مابيحرّكش قرش في أي خزنة، فسؤال «من أنهي خزنة؟» عليه
    // خطوة بتتسأل ومالهاش إجابة تفرق. ٢٠٢٦-٠٩-٢٠ بطلب صاحب النظام:
    // «لو مدفعش حاجه فالفاتوره ميختارش خزنه في البيع والشراء».
    //
    // ⚠️ **ودي عكس قرار أقدم منه** كان بيقول «سواء خصم وإضافة» — البوباب يظهر دايماً.
    // السبب وقتها إن الاختيار بيتسجّل `cash_account_id` على المستند، فلو المستند
    // اتعدّل بعدين ونزل عليه نقدي، بينزل في الخزنة اللي اتقالت يوم ما اتكتب.
    // بعد التغيير ده، المستند الآجل مابيشيلش خزنة — والنقدي اللي ينزل عليه بالتعديل
    // بيسأل ساعتها، أو السيرفر بيقرر. مكتوب هنا عشان اللي يرجّعها يعرف اتغيّرت ليه.
