...
describe('التحكم في القايمة نفسها', () => {
  ...
  it('الحذف إقفال مش مسح — والشاشة بتقول كده للي بيضغط', () => {
    // كانت بتمنع نداء `DELETE` أصلاً. ده كان صح لما مكانش فيه زرار حذف، وبقى بيقيس الحاجة
    // الغلط لما الزرار اتضاف: نقطة النهاية دي **بتقفل** الحساب (`active = False`) ومابتمسحش
    // الصف، فالقيود المترحّلة بتفضل لاقية اسمها.
    //
    // الخاصية دي بتاعة السيرفر، ومحلها `tests/integration/test_no_hard_delete.py` — بيعمل
    // حساب، بيحذفه، وبيتأكد إن الصف لسه موجود بإسمه. اللي بيتقاس هنا هو اللي الشاشة بتقوله:
    // «بيتقفل مش بيتمسح»، عشان اللي بيضغط يعرف إنه بيخفي مش بيدمّر.
    expect(subAccounts).toMatch(/بيتقفل مش بيتمسح/);
    expect(subAccounts).toMatch(/okButtonProps=\{\{ danger: true \}\}/);
  });

  it('حسابات النظام والمقفول أصلاً مش بيتحذفوا من الشاشة', () => {
    // السيرفر بيرفض حسابات النظام برضه — بس زرار شغّال بيرفض كل مرة هو زرار بيعلّم الناس
    // إن الشاشة مكسورة.
    expect(subAccounts).toMatch(/disabled=\{record\.is_system \|\| !record\.active\}/);
  });
