...
describe('اللي اتشال من الشرا بطلب العميل', () => {
  it('مفيش لوحة رصيد تحت الفاتورة', () => {
    // صندوق كبير مكتوب فيه «اختر فئة أو صنف عشان تشوف رصيده» وطالع الصفحة لتحت من غير
    // ما يقول حاجة. رصيد الصنف بيتشاف جوّه بوباب الاختيار.
    expect(buy, 'رجعت لوحة الرصيد').not.toContain('<ItemStockPanel');
    expect(sale, 'اتشالت من البيع كمان — ده مااتطلبش').toContain('<ItemStockPanel');
  });
});
