...
describe('نفس كثافة الشرا', () => {
  ...
  it('مفيش زراير كبيرة بتاكل ارتفاع', () => {
    // زرارين `large` في آخر الشاشة كانوا بياخدوا ارتفاع سطرين. المقصود **الزراير** —
    // الخانة الكبيرة جوّه بوباب بتتسأل مرة واحدة مالهاش دعوة بارتفاع الشاشة، فالفحص
    // بقى على `<Button` بالتحديد بدل ما يقع على أي `size="large"` في الملف.
    expect(src.replace(/<Select[\s\S]*?\/>/g, ''), 'لسه فيه زرار كبير')
      .not.toMatch(/<Button[^>]*size="large"/);
  });
});
